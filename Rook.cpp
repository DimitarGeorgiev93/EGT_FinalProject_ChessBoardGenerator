...
//constructor
Rook::Rook(bool chessPiece) : ChessPiece('R', chessPiece)
{
}

//moves method for a rook piece
bool Rook::moves(ChessPiece*** board) const
{
	int temp = 1;
	while (this->x + temp <= 7 ) {
...
//highligthMoves method for a rook piece
void Rook::highligthMoves(ChessPiece*** board, std::vector<std::pair<int, int>>& moves) const
{
	int temp = 1;
	while (this->x + temp <= 7) {
		if (board[this->x + temp][this->y] == nullptr) {
